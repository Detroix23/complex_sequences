//! # Complex sequences.
//! src/fractals/tables.rs

use std::ops::Rem;

use complex::{self, Complex};

use crate::structures::{color, computations};

/// Convert a 2D `table`: `Vec<Vec<State>>` into `Vec<u8>` of raw `data`. 
pub fn state_table_to_data(
	table: Vec<Vec<computations::State>>, 
	stable: color::Rgb, 
	divergent: color::Rgb,
	...
	for line in table {
		for state in line {
			match state {
				computations::State::Divergent{ iterations } => {
...
impl NewtonConverter {
	...
	fn match_root(self: &mut Self, root: computations::IsRoot) -> () {
		match root {
	...
	pub fn convert(
		self: &mut Self, 
		table: Vec<Vec<computations::IsRoot>>
	) -> computations::Data {
		for line in table {
			for root in line {
...
/// # `PositionConverter`: from color map table `Vec<Vec<Polar>>` to `Vec<u8>`.
...
impl PositionConverter {
	...
	pub fn convert(self: &mut Self, table: Vec<Vec<complex::Polar>>) -> computations::Data {
		for line in table {
			for z in line {
